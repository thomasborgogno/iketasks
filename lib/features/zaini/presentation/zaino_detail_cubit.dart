...
/// Per-zaino cubit: its items, tags and (derived) categories. See
/// [ZainoCubit] for the list-level cubit that owns the zaini themselves.
class ZainoDetailCubit extends Cubit<ZainoDetailState> {
  ...
  void _init() {
    ...
    _tagsSub?.cancel();
    _itemsSub = _repo
        .watchItems(uid, _zaino.id)
        .listen(
          (items) =>
              emit(state.copyWith(status: ZainoStatus.ready, items: items)),
          onError: (e) => emit(
            state.copyWith(
    ...
    _tagsSub = _repo
        .watchTags(uid, _zaino.id)
        .listen((tags) => emit(state.copyWith(tags: tags)));
  }
      ...
      emit(
        state.copyWith(
          items: [
            for (final i in state.items)
              if (i.id == item.id)
                i.copyWith(googleTaskId: googleTaskId)
              else
                i,
          ],
        ),
  ...
  Future<void> renameCategory(String oldName, String newName) async {
    ...
    final affected = state.items
        .where((i) => i.categoryName == oldName)
        .toList();
    ...
    emit(
      state.copyWith(
        items: [
          for (final i in state.items)
            if (i.categoryName == oldName)
              i.copyWith(categoryName: newName)
            else
              i,
        ],
      ),
        ...
        items: [
          for (final i in state.items)
            if (i.categoryName == name)
              i.copyWith(clearCategoryName: true)
            else
              i,
        ],
      ),
  ...
  Future<void> deleteItem(ZainoItem item) async {
    ...
    // Optimistic remove.
    emit(
      state.copyWith(items: state.items.where((i) => i.id != item.id).toList()),
    );
      ...
      state.copyWith(
        tags: [
          for (final t in state.tags)
            if (t.id == tag.id) t.copyWith(name: newName) else t,
        ],
        items: [
          for (final i in state.items)
            if (renamedById.containsKey(i.id))
              i.copyWith(tags: renamedById[i.id])
            else
              i,
        ],
        activeTagFilter: [
